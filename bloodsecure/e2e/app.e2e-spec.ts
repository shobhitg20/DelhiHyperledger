...
describe('Starting tests for bloodsecure', function() {
  ...
  it('website title should be bloodsecure', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('bloodsecure');
    })
  });

  it('network-name should be bloodsecur@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('bloodsecur@0.0.1.bna');
    });
  });

  it('navbar-brand should be bloodsecure',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('bloodsecure');
    });
  });
